...
class CalendarClient:
    ...
    def get_busy_times(self, start_date: datetime, end_date: datetime) -> List[Dict]:
        ...
        if not self.service:
            ...
            return [
                {
                    'start': start_date.replace(hour=10, minute=0).isoformat(),
                    'end': start_date.replace(hour=11, minute=0).isoformat(),
    ...
    def find_available_slots(self, start_date: datetime, end_date: datetime, 
        ...
        Args:
            ...
            business_hours_only: If True, only suggest slots during business hours (9 AM - 5 PM)
            ...
            if current_time < busy_start:
                ...
                if available_duration >= duration_minutes:
                    ...
                    while slot_start + timedelta(minutes=duration_minutes) <= busy_start:
                        if not business_hours_only or (9 <= slot_start.hour < 17):
        ...
        if current_time < end_date:
            available_duration = (end_date - current_time).total_seconds() / 60
            if available_duration >= duration_minutes:
                slot_start = current_time
